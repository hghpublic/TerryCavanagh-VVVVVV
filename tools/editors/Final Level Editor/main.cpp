...
namespace
{
  ...
  const std::string VVVVVV_ROOT = (std::getenv("VVVVVV_ROOT") ? std::getenv("VVVVVV_ROOT") : "");
  const fs::path DATA_DIR = (VVVVVV_ROOT.empty() ? fs::path() : fs::path(VVVVVV_ROOT) / "assets" / "game" / "data");
  const std::string TILES_PNG{(DATA_DIR / "graphics" / "tiles.png").string()};
  const std::string TILES2_PNG{(DATA_DIR / "graphics" / "tiles2.png").string()};
}
